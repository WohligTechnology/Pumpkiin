  ...
  /******/ [
    ...
    /***/ function(module, exports, __webpack_require__) {
      ...
      exports.toMoment = function(date, format, locale) {
        ...
        if (!exports.isValidMoment(momentDate)) momentDate = undefined;
        return momentDate;
      };
      exports.momentToValue = function(momentObject, format) {
        if (!exports.isValidMoment(momentObject)) return undefined;
      ...
      exports.valueToMoment = function(formattedValue, $scope) {
        ...
        if ($scope.model) {
          ...
          var views = $scope.views.all.slice(
            0,
      ...
      exports.setValue = function(value, $scope, $ctrl, $attrs) {
        var modelValue = exports.isValidMoment(value)
            ? value.clone()
            : exports.valueToMoment(value, $scope),
          viewValue = exports.momentToValue(modelValue, $scope.format);
        $scope.model = exports.updateMoment($scope.model, modelValue, $scope);
        $ctrl.$modelValue = exports.updateMoment(
          $ctrl.$modelValue,
          modelValue,
          $scope
        );
      ...
      exports.updateMoment = function(model, value, $scope) {
        if (!exports.isValidMoment(model) || !value) model = value;
        else {
      ...
      angular
        ...
        .directive("momentPicker", [
          ...
          function(
            ...
            return new directive_1["default"](
              $timeout,
              $sce,
              $log,
              $window,
    ...
    /***/ function(module, exports) {
      module.exports =
        '<div class=moment-picker> <div class="moment-picker-container {{view.selected}}-view" ng-class="{\'moment-picker-disabled\': disabled, open: view.isOpen}"> <div ng-if=additions.top class="moment-picker-addition top"></div> <table class=header-view ng-if=showHeader> <thead> <tr> <th ng-class="{disabled: !view.previous.selectable}" ng-bind-html=view.previous.label ng-click=view.previous.set()></th> <th ng-bind=view.title ng-click=view.setParentView()></th> <th ng-class="{disabled: !view.next.selectable}" ng-bind-html=view.next.label ng-click=view.next.set()></th> </tr> </thead> </table> <div class=moment-picker-specific-views> <table> <thead ng-if=views[view.selected].headers> <tr> <th ng-repeat="header in views[view.selected].headers" ng-bind=header></th> </tr> </thead> <tbody> <tr ng-repeat="row in views[view.selected].rows"> <td ng-repeat="item in row track by item.index" ng-class=item.class ng-bind=item.label ng-click="!disabled && views[view.selected].set(item)"></td> </tr> </tbody> </table> </div> <div ng-if=additions.bottom class="moment-picker-addition bottom"></div> </div> </div>';
        ...
        function Directive(
          ...
          this.link = function($scope, $element, $attrs, $ctrl, $transclude) {
            $transclude(function($transElement) {
              ...
              // limits
              $scope.limits = {
                minDate: utility_1.toMoment(
                  $scope.minDate,
                  $scope.format,
                  $scope.locale
                ),
                maxDate: utility_1.toMoment(
                  $scope.maxDate,
                  $scope.format,
                  $scope.locale
                ),
                ...
                isSelectable: function(value, precision) {
                  ...
                  try {
                    if (
                      angular.isFunction($scope.selectable) &&
                      $attrs["selectable"]
                    )
                ...
                checkValue: function() {
                  if (
                    !utility_1.isValidMoment($ctrl.$modelValue) ||
                    !$scope.validate
                  )
                    return;
                  if (!$scope.limits.isAfterOrEqualMin($ctrl.$modelValue))
                    utility_1.setValue(
                      $scope.limits.minDate,
                      $scope,
                      $ctrl,
                      $attrs
                    );
              ...
              $scope.views = {
                ...
                detectMinMax: function() {
                  ...
                  var minView, maxView;
                  angular.forEach($scope.views.formats, function(
                    formats,
                    view
                  ) {
              ...
              $scope.view = {
                ...
                update: function() {
                  $scope.view.value = utility_1.momentToValue(
                    $scope.view.moment,
                    $scope.format
                  );
                },
                ...
                position: function() {
                  ...
                  var element = $element[0],
                    ...
                    offset = helpers_1.getOffset($element[0]),
                    top = offset.top - _this.$window.pageYOffset,
                    left = offset.left - _this.$window.pageXOffset,
                    winWidth = _this.$window.innerWidth,
                    winHeight = _this.$window.innerHeight,
                    shouldHaveClassTop =
                      top + _this.$window.pageYOffset - picker.offsetHeight >
                        0 && top > winHeight / 2,
                ...
                keydown: function(e) {
                  var view = $scope.views[$scope.view.selected],
                    precision = $scope.views.precisions[
                      $scope.view.selected
                    ].replace("date", "day"),
                    singleUnit = _this.provider[precision + "sStep"] || 1,
                    operation =
                      [utility_1.KEYS.up, utility_1.KEYS.left].indexOf(
                        e.keyCode
                      ) >= 0
                        ? "subtract"
                        : "add",
                    highlight = function(vertical) {
                      var unitMultiplier = vertical ? view.perLine : 1,
                ...
                // utility
                unit: function() {
                  return $scope.view.selected == "decade" ? 10 : 1;
                },
                precision: function() {
                ...
                setParentView: function() {
                  $scope.view.change(
                    $scope.views.all[
                      Math.max(
                        0,
                        $scope.views.all.indexOf($scope.view.selected) - 1
                      )
                    ]
                  );
                },
                ...
                render: function() {
                  ...
                  $scope.view.previous.label = _this.$sce.trustAsHtml(
                    $scope.view.previous.selectable
                      ? $scope.leftArrow
                      : "&nbsp;"
                  );
                  ...
                  $scope.view.next.label = _this.$sce.trustAsHtml(
                    $scope.view.next.selectable ? $scope.rightArrow : "&nbsp;"
                  );
                  $scope.view.title = $scope.views[
                    $scope.view.selected
                  ].render();
                },
                change: function(view) {
                  ...
                  if (nextView < 0 || nextView > maxView) {
                    if (!$scope.setOnSelect) update();
                    if ($scope.autoclose) _this.$timeout($scope.view.close);
              ...
              ($scope.position || "").split(" ").forEach(function(className) {
                return $scope.picker.addClass(className);
              });
              if (!$scope.inline)
                $scope.picker[0].parentNode.removeChild($scope.picker[0]);
              else {
              ...
              _this.$timeout(function() {
                angular.forEach($scope.additions || {}, function(
                  tempalteUrl,
                  key
                ) {
              ...
              _this.$timeout(function() {
                if ($attrs["ngModel"]) {
                  ...
                  if ($scope.value)
                    $ctrl.$modelValue = utility_1.valueToMoment(
                      $scope.value,
                      $scope
                    );
                }
                // view initialization
                if ($scope.startDate)
                  $scope.view.moment = utility_1.toMoment(
                    $scope.startDate,
                    $scope.format,
                    $scope.locale
                  );
              ...
              if ($attrs["ngModel"]) {
                $ctrl.$parsers.push(function(viewValue) {
                  return (
                    utility_1.updateMoment(
                      $ctrl.$modelValue,
                      utility_1.valueToMoment(viewValue, $scope),
                      $scope
                    ) || true
                  );
                });
                $ctrl.$formatters.push(function(modelValue) {
                  return (
                    utility_1.momentToValue(modelValue, $scope.format) || ""
                  );
                });
              ...
              $scope.$watch(
                function() {
                  return utility_1.momentToValue(
                    $ctrl.$modelValue,
                    $scope.format
                  );
                },
                function(newViewValue, oldViewValue) {
                  if (newViewValue == oldViewValue) return;
                  var newModelValue = utility_1.valueToMoment(
                    newViewValue,
                    $scope
                  );
                  ...
                  if (angular.isFunction($scope.change) && $attrs["change"]) {
                    var oldModelValue_1 = utility_1.valueToMoment(
                      oldViewValue,
                      $scope
                    );
              ...
              $scope.$watch(
                function() {
                  return $ctrl.$modelValue && $ctrl.$modelValue.valueOf();
                },
                function() {
              ...
              $scope.$watch("view.selected", function() {
                return $scope.view.render();
              });
              ...
              $scope.$watchGroup(
                ...
                function() {
                  angular.forEach(["minDate", "maxDate"], function(field) {
                    $scope.limits[field] = utility_1.toMoment(
                      $scope[field],
                      $scope.format,
                      $scope.locale
                    );
              ...
              $scope.$watch(
                ...
                function(newViewValue, oldViewValue) {
                  if (newViewValue == oldViewValue) return;
                  $scope.view.moment = utility_1.valueToMoment(
                    newViewValue,
                    $scope
                  );
                  $scope.view.update();
              ...
              $attrs.$observe("locale", function(locale) {
              ...
              $scope.$watch("isOpen", function(isOpen) {
                if ($scope.inline) $scope.view.isOpen = true;
                  ...
                  angular.isDefined(isOpen) &&
                  isOpen != $scope.view.isOpen
                )
                  $scope.view.toggle();
              });
              // event listeners
              ...
              $scope.input
                .on("focus click touchstart", function() {
                  return $scope.$evalAsync($scope.view.open);
                })
                .on("blur", function() {
                  return $scope.$evalAsync($scope.view.close);
                })
              ...
              $scope.container.on("mousedown", function(e) {
                return focusInput(e);
              });
              angular
                .element(_this.$window)
                .on("resize scroll", $scope.view.position);
      ...
      var DayView = (function() {
        ...
        DayView.prototype.render = function() {
          ...
          for (
            ...
            var index = Math.floor(h / this.perLine),
            ...
            this.rows[index].push({
              ...
              class: [
                this.$scope.keyboard &&
                hour.isSame(this.$scope.view.moment, "hour")
                  ? "highlighted"
                  : "",
                !selectable
                  ? "disabled"
    ...
    /***/ function(module, exports, __webpack_require__) {
      ...
      var DecadeView = (function() {
        ...
        DecadeView.prototype.render = function() {
          ...
          year.year(firstYear);
          for (var y = 0; y < 12; y++) {
            var index = Math.floor(y / this.perLine),
            ...
            this.rows[index].push({
              ...
              class: [
                this.$scope.keyboard &&
                year.isSame(this.$scope.view.moment, "year")
                  ? "highlighted"
                  : "",
      ...
      var HourView = (function() {
        ...
        HourView.prototype.render = function() {
          ...
          for (
            ...
            var index = Math.floor(i / this.perLine),
        ...
        HourView.prototype.highlightClosest = function() {
          ...
          angular.forEach(this.rows, function(row) {
            angular.forEach(row, function(value) {
              if (
                Math.abs(value.minute - _this.$scope.view.moment.minute()) <
      ...
      var MinuteView = (function() {
        ...
        MinuteView.prototype.highlightClosest = function() {
          ...
          angular.forEach(this.rows, function(row) {
            angular.forEach(row, function(value) {
              if (
                Math.abs(value.second - _this.$scope.view.moment.second()) <
          ...
          for (var week = firstWeek; week <= lastWeek; week++)
            rows[week] = Array.apply(null, Array(this.perLine)).map(function() {
              ...
              var date = {
                ...
                date: day.date(),
                class: [
                  _this.$scope.keyboard &&
                  day.isSame(_this.$scope.view.moment, "day")
                    ? "highlighted"
                    : "",
                  !!_this.$scope.today && day.isSame(new Date(), "day")
                    ? "today"
                    : "",
      ...
      var YearView = (function() {
        ...
        YearView.prototype.render = function() {
          ...
          months.forEach(function(label, i) {
            var index = Math.floor(i / _this.perLine),
              selectable = _this.$scope.limits.isSelectable(month, "month");
            if (!_this.rows[index]) _this.rows[index] = [];
            _this.rows[index].push({
              ...
              class: [
                _this.$scope.keyboard &&
                month.isSame(_this.$scope.view.moment, "month")
                  ? "highlighted"
                  : "",
                !selectable
                  ? "disabled"
